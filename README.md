...
| `simple_spread2.py` (Cooperative navigation) | N | N | N agents, N landmarks. Agents are rewarded based on how far any agent is from each landmark. Agents are penalized if they collide with other agents. And they do not share one common reward. So, agents have to learn to cover all the landmarks while avoiding collisions. |
| `simple_reference2.py` | Y | N | 2 agents, 3 landmarks of different colors. Each agent wants to get to their target landmark, which is known only by other agent. Reward is not collective. They want to cover their own landmarks but pay more attention to help other. Agents have to learn to communicate the goal of the other agent, and navigate to their landmark. This is the same as the simple_speaker_listener scenario where both agents are simultaneous speakers and listeners. |
